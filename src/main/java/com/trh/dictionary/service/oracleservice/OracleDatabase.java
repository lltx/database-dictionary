...
public class OracleDatabase {
    ...
    public static List<TableInfo> getTableInfo() {
        ...
        try {
            //查询数据库下所有的表名
            pstmt = OracleJdbc.getConn().prepareStatement("select t.table_name from user_tables t");
            //建立一个结果集，用来保存查询出来的结果
            ...
            while (rs.next()) {
                ...
                //根据表名查询主键列名
                PreparedStatement pstmts = OracleJdbc.getConn().prepareStatement("SELECT column_name FROM all_ind_columns WHERE TABLE_NAME ='"+tableName+"'");
                ResultSet ResultName = pstmts.executeQuery();
                //通过表名查询所有的列数据
                pstmt = OracleJdbc.getConn().prepareStatement("SELECT T.column_id,T.column_name,T.data_type,T.data_default,T.nullable,b.comments\n" +
                ...
                //查询表的索引说明
                pstmt = OracleJdbc.getConn().prepareStatement("SELECT rownum,rs.* FROM (SELECT leftTable.*,CASE b.constraint_type WHEN  'P' THEN '1' ELSE '0'END AS isIndex \n" +
